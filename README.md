...
| 시각화 종류     | 설명 |
|----------------|------|
| 🥧 **파이 차트**     | 미디어 채널 가입 여부와 사용 여부 비율 분포 시각화 |
| 📉 **히스토그램**   | 나이대별 분석 |
| 📊 **스택 막대 차트** | 성별별 채널 이용률 분석 |
| 📈 **영역 차트**     | 등급별 채널 이용률 변화 |
| 🧮 **테이블 차트**   | 지역 + 카테고리별 채널 이용률 매트릭스 형태 분석 |
| 🗺️ **지도 시각화**   | 지역별 채널 이용률 |
...
- 전체 고객 중 **65.05%**가 디지털(미디어) 채널에 가입함
- 디지털 가입률이 과반 이상이며, **디지털 환경에 익숙한 고객이 다수** 존재함을 시사
- 그러나 **34.95%는 비가입 상태**로, 여전히 오프라인 중심 고객군도 존재
...
- 성별 코드 기준:  
  - 1 = 남성  
  - 2 = 여성
- 여성 고객의 디지털 채널 가입률이 상대적으로 **높게 나타남**
- 마케팅 시 성별 특성에 따른 접근 전략 필요
...
- 사용률: Y (이용) 36.94% / N (미이용) 63.06%
- 실제 사용 비율은 **가입률보다 낮음**
- 가입 이후 **실질적 사용 유도**를 위한 전략 필요
...
- 성별별로 볼 때, **여성 고객의 채널 사용률이 높음**
- 디지털 콘텐츠에 대한 성별 선호 차이가 반영된 결과로 해석 가능
...
- 높은 등급일수록 디지털 채널 사용률이 높음 (ex. 등급 24~25)
- **로열 고객군이 디지털 채널을 적극적으로 이용**하는 경향 존재
- 등급이 낮은 고객의 채널 유입 전략 필요
...
- **30~50대에서 가장 높은 사용률**을 보임
- 60대 이상 고령층에서는 사용률이 현저히 낮음
- 연령 기반 UX 개선 및 비고객층 디지털 전환 캠페인 필요
...
- `음식/외식`, `교육/자녀교육`, `정보/통신` 등 일상 소비 중심 카테고리에서 디지털 채널 이용 비중이 높음
- 반면, `기타공과금`, `사무/세무/법률` 카테고리는 이용률이 낮음
- **소비 유형에 따라 디지털 채널의 효용 인식 차이** 존재
- 
